...
for j in range(len(ID)):
    ...
    while len(stars) == 0 and radii < 0.7:
    	print('No stars within the selected radius')
    	radii += 0.05
    	if diff_sort[0] == 0:
    		stars = np.where(dist == 0)[0]
    	else:
	...
	file_spectra = ret.set_spectra_name(Teff_new, np.log(g_new), Z_new, IDcomp)
...
plt.figure()
plot2 = plt.scatter(Teff, Z, c=logg)
plt.colorbar(plot2)
plt.xlabel('Teff')
plt.ylabel('Z')
plt.gca().invert_xaxis()
